                        ...
                        <div class="table-responsive p-0">
                            <table class="table align-items-center mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="ps-4 text-nowrap"><p class="text-xs text-secondary mb-0">{{ $location->id }}</p></td>
                                        <td class="text-nowrap"><p class="text-xs font-weight-bold mb-0">{{ $location->name }}</p></td>
                                        <td class=""><p class="text-xs text-secondary mb-0">{{ $location->description }}</p></td>
                                        <td class="align-middle text-nowrap">
                                            <!-- Edit Button -->
                                            <a href="javascript:;" class="text-secondary font-weight-bold text-xs mr-2" data-toggle="modal" data-target="#editCategoryModal-{{ $location->id }}">
                                                Edit
                                            </a>
                                            <!-- Delete Button -->
                                    ...
                                    <!-- Edit Modal -->
                                    <div class="modal fade" id="editCategoryModal-{{ $location->id }}" tabindex="-1" role="dialog" aria-labelledby="editCategoryModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered" role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                ...
                                                <form role="form" action="{{ route('locations.update', $location->id) }}" method="POST">
                                                    ...
                                                    <div class="modal-body">
                                                        <div class="form-group">
                                                            <label for="name-{{ $location->id }}" class="text-sm font-weight-bold text-dark">Name</label>
                                                            <input type="text" name="name" id="name-{{ $location->id }}" class="form-control" value="{{ $location->name }}" required>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="description-{{ $location->id }}" class="text-sm font-weight-bold text-dark">Description</label>
